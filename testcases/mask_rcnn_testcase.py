...
def save_img(arr,fname,recshapes):
...
def unmold_detections(detections, mrcnn_mask, original_image_shape,
    ...
    # for box in boxes:
    #     print("area ck", box, (box[2]-box[0]) * (box[3]-box[1]))
...
def batch_slice(inputs, graph_fn, batch_size, names=None):
    ...
    """
    print("[info] batch slice")
    print("inputs",len(inputs))
    print("graph",graph_fn)
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    print('deltas_specific.shape',deltas_specific.shape)
    ...
    if config['DETECTION_MIN_CONFIDENCE']:
        ...
        print('conf_keep',len(conf_keep),conf_keep)
    ...
    print('refined_rois',refined_rois.shape)
    ...
    print('unique_pre_nms_class_ids',unique_pre_nms_class_ids.shape)
    ...
    def nms_keep_map(class_id,pre_nms_class_ids,pre_nms_rois,pre_nms_scores):
        ...
        for i in range(len(ixs)):
            ...
            print('nms boxes.shape',boxes.shape,boxes)
            print('nms scores.shape',scores.shape,scores)
            ...
            print('ixs[i]',ixs[i].shape,ixs[i])
            print('class_keep',class_keep.shape,class_keep)

            # Map indices
            class_keep_idx = np.take(ixs[i], class_keep)
            print('class_keep_idx',class_keep.shape,keep)
...
def single_img_test(data,test_generator,model,testconfig):
    ...
    print("[debug] Generate predictions for some samples")
    ...
    print(img_id,ori_img_arr.shape,ori_img_arr)
    save_img(ori_img_arr,"single_img_test",[])
    #TODO: cov to detections
    ...
    print("mold_img_shape",mold_img_shape)
    print("window",window)
    '''
    detections: [N, (y1, x1, y2, x2, class_id, score)] in normalized coordinates
    mrcnn_mask: [N, height, width, num_classes]
    ...
    print("ck predictions len",len(predictions))
    print("ck mrcnn_mask shape",mrcnn_mask.shape)
    print("ck img_meta",inputs[1][0])

    print("ck mrcnn_class_logits",mrcnn_class_logits.shape)
    print("ck mrcnn_class",mrcnn_class.shape)
    print("ck mrcnn_bbox",mrcnn_bbox.shape)

    # detections = []
    # for index in range(len(mrcnn_class_logits)):
    #     class_id = np.argmax(mrcnn_class[index])
    #     score = mrcnn_class[index][class_id]
    #     y1,x1,y2,x2 = mrcnn_bbox[index][class_id]
    #     detections.append((y1, x1, y2, x2, class_id, score))
    # detections = np.array(detections)

    print("detections",detections.shape)
    print("mrcnn_mask",mrcnn_mask.shape)
    print("ori_img_shape",ori_img_shape.shape)
    print("mold_img_shape",mold_img_shape.shape)
    ...
    test_img = np.array(np.array(inputs[0][0]) + np.array(testconfig["MEAN_PIXEL"]),dtype=np.int32)
    # print(test_img[123])
    print("mrcnn_class",mrcnn_class.shape,mrcnn_class[0][1])
    print("mrcnn_bbox",mrcnn_bbox.shape,mrcnn_bbox[0][1])
    print("output_rois",output_rois.shape,output_rois[0][1])
    roi = output_rois[0][1]
    h = 255
    w = 255
    scale = np.array([h, w, h, w]) - 1.0
    ...
    y1, x1, y2, x2 = np.multiply(roi,scale) + shift
    print(y1, x1, y2, x2)
    a_center_y = (y2+y1)/2
    a_center_x = (x2+x1)/2
    print("a_center",a_center_y,a_center_x)
    p1,p2,p3,p4 = mrcnn_bbox[0][1][1]
    # p3 = np.log(gt_h / a_h)
    a_h = np.exp(np.log(255) - p3)
    # p4 = np.log(gt_w / a_w)
    a_w = np.exp(np.log(255) - p4)
    # (gt_center_y - a_center_y) / a_h,
    gt_center_y = p1 * a_h + a_center_y
    # (gt_center_x - a_center_x) / a_w
    gt_center_x = p2 * a_w + a_center_x
    print(a_h,a_w,gt_center_x,gt_center_y)
    # x0, y0, x1, y1
    recshape = [a_center_x - a_w/2, a_center_y - a_h/2 , a_center_x + a_w/2, a_center_y + a_h/2]
    print("recshape",recshape)
    save_img(test_img,"single_img_test",recshape)
...
def mytest(**args):
    ...
    if opt_set.opt_test:
        ...
        single_img_test(data,test_generator,model,testconfig)
